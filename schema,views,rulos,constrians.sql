...
--same schema can exist in differint DBs and point to many tables
...
--default schema in sql DBs is dbo

-----------------Don't forget to change schema using wizerd---------------
--from table properties then schema lable

------------------------synonym-----------------------------------------
--what is synonym ? ----Saved alias name for database objects 
create synonym  s1
for hr.student
...
--one column can have several constrains 
-- constrains apply only in the specified table 
...
--Note Constraint apply on old and new data 
...
--what if we want to write Constraint once and make it shaired
--               on more than one column and in differnt table
